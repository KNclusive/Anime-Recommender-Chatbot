...
To run the project you have to run the trainer.ipynb to create nessesary ".joblib" file either in google drive or local (Preferably in google drive so that the project remains device agnostic). Then one can run Chatbot_Flask.ipynb to create the local host server where user can interract with the recommender system.
